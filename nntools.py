...
class Experiment(object):
    ...
    def adjust_learning_rate(self, optimizer, epoch, cnt_rate):
        if epoch==0:
            lr = 0.0001 * (cnt_rate*9+1)
        elif epoch<30:
            lr = 0.001
        elif epoch<105:
            lr = 0.0001
    ...
    def run(self, num_epochs, plot=None):
        ...
        num_of_batches = len(self.train_loader)
        for epoch in range(start_epoch, num_epochs):
            ...
            for x, d in self.train_loader:
                if cnt != 0 and cnt % 10 == 0:
                    print('------------------------------------------------')
                    print('{}/{} ({}%) batches finished.'.format(cnt, num_of_batches, int(cnt*100.0/num_of_batches)))
                    print('forward_time: {}/{} ({}%)'.format(round(forward_time, 3), round(total_time, 3),
                                                             int(forward_time*100.0/total_time)))
                    print('criterion_time: {}/{} ({}%)'.format(round(criterion_time, 3), round(total_time, 3),
                                                             int(criterion_time * 100.0 / total_time)))
                    print('backward_time: {}/{} ({}%)'.format(round(backward_time, 3), round(total_time, 3),
                                                             int(backward_time * 100.0 / total_time)))
                    forward_time = 0
                    criterion_time = 0
                    backward_time = 0
                    total_time = 0
                ...
                self.adjust_learning_rate(self.optimizer, epoch, cnt/num_of_batches)
                ...
                with torch.no_grad():
                    self.stats_manager.accumulate(loss.item(), x, y, d, cnt, eval_mode='train')
                cnt += 1
                total_time += time.time() - t00
            if not self.perform_validation_during_training:
                self.history.append(self.stats_manager.summarize(eval_mode='train'))
            else:
                self.history.append(
                    (self.stats_manager.summarize(eval_mode='train'), self.evaluate(eval_mode='train')))
            #self.scheduler.step()
    ...
    def evaluate(self, eval_mode):
        ...
        with torch.no_grad():
            ...
            for x, d in self.val_loader:
                ...
                y = self.net.forward(x)
                loss = self.net.criterion(y, d)
                self.stats_manager.accumulate(loss.item(), x, y, d, cnt, eval_mode)
                cnt += 1
        self.net.train()
        return self.stats_manager.summarize(eval_mode)
